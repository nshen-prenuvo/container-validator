...
def run_preprocessing(t1_path, t2_path, output_dir, config_path="/app/model_config.json"):
    ...
    # Debug: Check what's in the pyenv directory
    pyenv_root = Path("/root/.pyenv")
    if pyenv_root.exists():
        logger.info(f"Pyenv root exists at {pyenv_root}")
        versions_dir = pyenv_root / "versions"
        if versions_dir.exists():
            logger.info(f"Pyenv versions directory exists at {versions_dir}")
            available_versions = list(versions_dir.iterdir())
            logger.info(f"Available pyenv versions: {[v.name for v in available_versions]}")
        else:
            logger.error(f"Pyenv versions directory not found at {versions_dir}")
    else:
        logger.error(f"Pyenv root not found at {pyenv_root}")
    
    if not python_path.exists():
        logger.error(f"Python installation not found at {python_path}")
        # Try to find alternative Python in pyenv
        if pyenv_root.exists():
            versions_dir = pyenv_root / "versions"
            if versions_dir.exists():
                # Look for any Python executable in pyenv
                for version_dir in versions_dir.iterdir():
                    potential_python = version_dir / "bin" / "python"
                    if potential_python.exists():
                        python_path = potential_python
                        logger.info(f"Found alternative pyenv Python at {python_path}")
                        break
                else:
                    logger.error("No Python executable found in any pyenv version")
                    return False
            else:
                return False
        else:
    ...
    if result.returncode != 0:
        ...
        if result.stdout:
            logger.error(f"Preprocessing output: {result.stdout}")
        return False
    
    # Log the output even on success to see what happened
    if result.stdout:
