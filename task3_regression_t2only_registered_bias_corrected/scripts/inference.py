...
class BiasCorrection:
    """Bias correction using Cole's method for brain age predictions"""
    
    def __init__(self):
        # Linear regression fit from 700 healthy training subjects
        self.cole_a = 0.532
        self.cole_b = 28.354

    def cole_correction(self, pred_age):
        """
        Apply Cole's correction to predicted brain age
        
        Args:
            pred_age: Raw predicted brain age
            
        Returns:
            Corrected brain age prediction
        """
        corrected_pred_age = (pred_age - self.cole_b) / self.cole_a
        return corrected_pred_age
...
class FOMORegressionInferenceEngine:
    """Modular inference engine for FOMO25 Task 3 regression"""
    ...
    def preprocess_data(self, data_array: np.ndarray) -> torch.Tensor:
        ...
        """
        # Extract T2 modality (only modality available)
        t2_data = data_array[0]  # T2 is the only modality at index 0
        ...
        if t2_tensor.dim() == 3:  # (H, W, D)
            t2_tensor = t2_tensor.unsqueeze(0)  # Add channel dimension
            t2_tensor = t2_tensor.unsqueeze(0)  # Add batch dimension
        elif t2_tensor.dim() == 4:  # (C, H, W, D)
            t2_tensor = t2_tensor.unsqueeze(0)  # Add batch dimension
    ...
    def run_inference(self, t2_tensor: torch.Tensor) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            t2_values = t2_predictions.cpu().numpy()
    ...
    def predict_single_subject(self, data_array: np.ndarray, properties: dict = None) -> tuple:
        ...
        Args:
            data_array: Preprocessed data array (T2 modality only)
            properties: Properties dictionary with metadata
            
        Returns:
            Tuple of (t2_predictions, properties_with_predictions)
        """
        # Preprocess data for T2 modality only
        t2_tensor = self.preprocess_data(data_array)
        
        # Run inference on T2 modality
        t2_values = self.run_inference(t2_tensor)
        
        # Apply Cole's correction
        bias_corrector = BiasCorrection()
        corrected_t2_values = bias_corrector.cole_correction(t2_values)
        
        # Add both raw and corrected T2 predictions to properties for saving
        if properties is None:
            properties = {}
        properties['t2_predictions'] = t2_values
        properties['corrected_t2_predictions'] = corrected_t2_values
        
        return corrected_t2_values, properties
...
def save_regression_results(t2_predictions: np.ndarray,
                           output_path: str,
                           subject_name: str = None):
    """
    Save regression results for T2 modality brain age prediction
    
    Args:
        t2_predictions: T2 modality regression predictions (brain age)
    ...
    # Get the brain age prediction from T2
    brain_age = t2_predictions[0, 0]
    
    # Save the T2 brain age prediction
    with open(final_path, 'w') as f:
        f.write(f"{brain_age:.1f}")
    
    # Log prediction for monitoring/debugging
    logger.info(f"Brain age prediction (T2 only) saved to {final_path}: {brain_age:.1f} years")

def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="FOMO25 Task 3 Brain Age Regression Inference (T2 modality only)")
...
def main():
    ...
    try:
        ...
        t2_predictions, properties = inference_engine.predict_single_subject(data, properties)
        
        # Extract both raw and corrected T2 predictions from properties
        raw_t2_predictions = properties.get('t2_predictions')
        corrected_t2_predictions = properties.get('corrected_t2_predictions')
        
        if corrected_t2_predictions is None:
            logger.error("Corrected T2 modality predictions not found in properties")
            sys.exit(1)
        
        # Log both raw and corrected predictions for transparency
        raw_brain_age = raw_t2_predictions[0, 0] if raw_t2_predictions is not None else "N/A"
        corrected_brain_age = corrected_t2_predictions[0, 0]
        logger.info(f"Raw T2 brain age prediction: {raw_brain_age:.1f}")
        logger.info(f"Corrected T2 brain age prediction (Cole's method): {corrected_brain_age:.1f}")
        
        # Save result
        logger.info("Saving brain age prediction results (T2 modality only)...")
        save_regression_results(corrected_t2_predictions, args.output)
        ...
        brain_age = corrected_t2_predictions[0, 0]
        
        print(f"T2 Prediction: {brain_age:.1f}")
